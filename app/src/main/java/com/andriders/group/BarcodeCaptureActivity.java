...
import com.andriders.group.camera.CameraSource;
import com.andriders.group.camera.CameraSourcePreview;
...
public final class BarcodeCaptureActivity extends AppCompatActivity {
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        if (grantResults.length != 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
            ...
            // we have permission, s
            // o create the camerasource
            boolean autoFocus = getIntent().getBooleanExtra(AutoFocus,true);
            boolean useFlash = getIntent().getBooleanExtra(UseFlash, false);
            createCameraSource(autoFocus, useFlash);
            return;
    ...
    private class BarcodeTracker extends Tracker<Barcode> {
        ...
        public void onNewItem(final int id, final Barcode item) {
            BarcodeCaptureActivity.this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (item.rawValue.startsWith(RiderGroup.GROUP_NAME_PREFIX)) {
                        Toast.makeText(BarcodeCaptureActivity.this, "" + id + " : " + item.rawValue,
                                Toast.LENGTH_LONG).show();
                        Intent intent = new Intent();
                        intent.putExtra(EXTRA_GROUP_NAME, item.rawValue);
